...
def test_scrape_error_handling():
    ...
    result = scrape_url("https://invalid-url-that-does-not-exist")
    assert result["status"] == 404 or result["status"] == 500
    assert "error" in result["content"] or "Failed to fetch content" in result["content"]