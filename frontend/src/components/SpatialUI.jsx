    ...
    <aside className="telemetry glass" aria-label="System telemetry">
      ...
      <div className="telemetry-section">
        ...
        <div className="metric">
          ...
          <strong>{hardware?.gpu_available ? `${hardware.gpu_percent}%` : "Unavailable"}</strong>
        </div>
        <div className="meter"><span style={{ width: `${Math.min(100, Math.max(0, hardware?.gpu_percent || 0))}%` }} /></div>
        <div className="metric">
          <span>GPU memory (VRAM)</span>
          <strong>{hardware?.gpu_available ? `${hardware.gpu_memory_used_gb} / ${hardware.gpu_memory_total_gb} GB` : "Unavailable"}</strong>
        </div>
        <div className="meter"><span style={{ width: `${Math.min(100, Math.max(0, hardware?.gpu_memory_percent || 0))}%` }} /></div>
        {!hardware?.gpu_available && <p>NVIDIA GPU telemetry requires the NVIDIA driver tools.</p>}
      </div>
      <div className="telemetry-section">
